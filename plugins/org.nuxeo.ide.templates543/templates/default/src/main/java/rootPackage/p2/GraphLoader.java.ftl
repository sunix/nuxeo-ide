...
import org.eclipse.ecr.build.Artifact;
import org.eclipse.ecr.build.Dependency;
import org.eclipse.ecr.build.Graph;
import org.eclipse.ecr.build.Node;
        ...
        while (node != null) {
            if (node.getNodeType() == org.w3c.dom.Node.ELEMENT_NODE) {
                if ("required".equals(node.getNodeName())) {
                    Element element = (Element)node;
                    String ns = element.getAttribute("namespace");
                    Dependency imp = new Dependency(element.getAttribute("name"),
